...
for (const chart of CHARTS) {
  for (const theme of THEMES) {
    for (const [sizeName, { w, h }] of Object.entries(SIZES)) {
      const ref = `http://localhost:4300/?type=radar&w=${w}&h=${h}&theme=${theme}&chart=${chart}`;
      const ours = `http://localhost:4500/#/iso/radar?w=${w}&h=${h}&chart=${chart}`;
      const tag = `radar-${chart}-${theme}-${sizeName}`;

      test(`radar [${tag}] static`, async ({ browser }) => {
        ...
        await capture(p2, ours, theme, '.iso-chart', 'spn-radar-chart svg', path.join(OUT, `ours-${tag}-static.png`));
