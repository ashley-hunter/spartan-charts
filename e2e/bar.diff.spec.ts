...
const CHARTS = ['default', 'horizontal', 'multiple', 'label', 'stacked', 'mixed', 'negative', 'active', 'label-custom', 'interactive'] as const;
...
for (const chart of CHARTS) {
 for (const theme of THEMES) {
  for (const [sizeName, { w, h }] of Object.entries(SIZES)) {
    const ref = `http://localhost:4300/?type=bar&w=${w}&h=${h}&theme=${theme}&chart=${chart}`;
    const ours = `http://localhost:4500/#/iso/bar?w=${w}&h=${h}&chart=${chart}`;
    const tag = `bar-${chart}-${theme}-${sizeName}`;

    test(`bar [${tag}] static`, async ({ browser }) => {
      ...
      await capture(p2, ours, theme, '.iso-chart', 'spn-bar-chart svg', path.join(OUT, `ours-${tag}-static.png`));
    ...
    test(`bar [${tag}] hover`, async ({ browser }) => {
      fs.mkdirSync(OUT, { recursive: true });
      const p1 = await browser.newPage({ deviceScaleFactor: 2 });
      await capture(p1, ref, theme, '.chart', '.recharts-surface', path.join(OUT, `ref-${tag}-hover.png`), 0.4);
      const p2 = await browser.newPage({ deviceScaleFactor: 2 });
      await capture(p2, ours, theme, '.iso-chart', 'spn-bar-chart svg', path.join(OUT, `ours-${tag}-hover.png`), 0.4);
