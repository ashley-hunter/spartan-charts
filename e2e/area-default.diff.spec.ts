...
const CHARTS = ['default', 'linear', 'step', 'stacked', 'expand', 'legend', 'icons', 'gradient', 'axes', 'interactive'] as const;
...
// Honest diff metric: mismatch as a fraction of "inked" pixels (those that differ
// from the background), not of the whole mostly-empty image.
function diffPng(aPath: string, bPath: string, outPath: string): string {
  ...
  // background = top-left corner of reference; count inked pixels in reference
...
for (const chart of CHARTS) {
 for (const theme of THEMES) {
  for (const [sizeName, { w, h }] of Object.entries(SIZES)) {
    const ref = `http://localhost:4300/?w=${w}&h=${h}&theme=${theme}&chart=${chart}`;
    const ours = `http://localhost:4500/#/iso/area?w=${w}&h=${h}&chart=${chart}`;
    const tag = `${chart}-${theme}-${sizeName}`;

    test(`area default [${tag}] static`, async ({ browser }) => {
      ...
      await capture(p2, ours, theme, '.iso-chart', 'spn-area-chart svg', path.join(OUT, `ours-${tag}-static.png`));
    ...
    test(`area default [${tag}] hover`, async ({ browser }) => {
      fs.mkdirSync(OUT, { recursive: true });
      const p1 = await browser.newPage({ deviceScaleFactor: 2 });
      await capture(p1, ref, theme, '.chart', '.recharts-surface', path.join(OUT, `ref-${tag}-hover.png`), 0.4);
      const p2 = await browser.newPage({ deviceScaleFactor: 2 });
      await capture(p2, ours, theme, '.iso-chart', 'spn-area-chart svg', path.join(OUT, `ours-${tag}-hover.png`), 0.4);
