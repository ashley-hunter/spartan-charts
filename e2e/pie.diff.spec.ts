...
for (const chart of CHARTS) {
 for (const theme of THEMES) {
  for (const [sizeName, { w, h }] of Object.entries(SIZES)) {
    const ref = `http://localhost:4300/?type=pie&w=${w}&h=${h}&theme=${theme}&chart=${chart}`;
    const ours = `http://localhost:4500/#/iso/pie?w=${w}&h=${h}&chart=${chart}`;
    const tag = `pie-${chart}-${theme}-${sizeName}`;

    test(`pie [${tag}] static`, async ({ browser }) => {
      ...
      await capture(p2, ours, theme, '.iso-chart', 'spn-pie-chart svg', path.join(OUT, `ours-${tag}-static.png`));
    ...
    test(`pie [${tag}] hover`, async ({ browser }) => {
      fs.mkdirSync(OUT, { recursive: true });
      const p1 = await browser.newPage({ deviceScaleFactor: 2 });
      await capture(p1, ref, theme, '.chart', '.recharts-surface', path.join(OUT, `ref-${tag}-hover.png`), 0.5);
      const p2 = await browser.newPage({ deviceScaleFactor: 2 });
      await capture(p2, ours, theme, '.iso-chart', 'spn-pie-chart svg', path.join(OUT, `ours-${tag}-hover.png`), 0.5);
