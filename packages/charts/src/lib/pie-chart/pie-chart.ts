...
export class SpnPieChart<T = unknown> implements AfterContentInit {
  ...
  constructor() {
    // Disable cursor line for pie charts (doesn't make sense for radial charts)
    this.chartContext.showCursorLine.set(false);
  ...
  private renderPie(pie: SpnPie<T>, pieIndex: number): void {
    ...
    const pieGenerator = d3
      ...
      .startAngle(this.degreesToRadians(pie.startAngle()))
      .endAngle(this.degreesToRadians(pie.endAngle()))
    ...
    const pieGroup = d3
      .select(this.piesLayer!)
      .append('g')
      .attr('class', `pie-${pieIndex}`);

    // Animation settings
    const isAnimationActive = pie.isAnimationActive();
    ...
    arcs.forEach((arc, i) => {
      ...
      if (isAnimationActive) {
        ...
        path
          ...
          .delay(i * 50)
          .attrTween('d', () => (t) => arcGenerator(interpolate(t) as PieArcData<T>) || '');
      } else {
        path.attr('d', arcGenerator(arc) || '');
      }
    });
    ...
    this.registerPieSectors(pie, arcs, colors);
  }
  ...
  private renderLabels(
    ...
    const labelConfig = pie.label();
    const config: PieLabelConfig =
      typeof labelConfig === 'boolean' ? {} : (labelConfig as PieLabelConfig);
    ...
    const labelsGroup = d3
      .select(this.labelsLayer!)
      .append('g')
      .attr('class', `labels-${pieIndex}`);
    ...
    arcs.forEach((arc, i) => {
      ...
      if (position === 'outside') {
        ...
        if (labelLineConfig) {
          const lineConfig: PieLabelLineConfig =
            typeof labelLineConfig === 'boolean' ? {} : labelLineConfig;
          const lineStroke = lineConfig.stroke || colors[i % colors.length];
          const lineStrokeWidth = lineConfig.strokeWidth ?? 1;

          const outerPoint = [
            Math.sin(midAngle) * (outerRadius + 5),
            -Math.cos(midAngle) * (outerRadius + 5),
          ];
          const labelPoint = [
            Math.sin(midAngle) * (labelRadius - 5),
            -Math.cos(midAngle) * (labelRadius - 5),
          ];
  ...
  private registerPieSectors(pie: SpnPie<T>, arcs: PieArcData<T>[], colors: string[]): void {
    const nameKey = pie.nameKey();

    arcs.forEach((arc, i) => {
      const name = nameKey
        ? String(getValueByDataKey(arc.data, nameKey))
        : pie.name() || `Sector ${i + 1}`;
      const color = colors[i % colors.length];

      this.chartContext.registerGraphicalElement({
        id: `pie-sector-${i}-${Math.random().toString(36).substr(2, 5)}`,
