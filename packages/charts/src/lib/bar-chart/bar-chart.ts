...
export class SpnBarChart<T = unknown> implements AfterContentInit {
  // Signal-based inputs
  ...
  readonly barCategoryGap = input<string | number>('10%');
  readonly barSize = input<number>();
  ...
  private calculateValueAxisDomain(
    data: ChartData<T>,
    domainInput: Domain,
  ): [number, number] {
    if (domainInput[0] !== 'auto' && domainInput[0] !== 'dataMin') {
  ...
  private parsePadding(value: string | number): number {
  ...
  private renderStackedBars(
    ...
    const categoryDataKey = isHorizontal
      ? this.yAxes()[0]?.dataKey() || ''
      : this.xAxes()[0]?.dataKey() || '';
    ...
    const barWidth = (bandwidth - barGap * (totalSeries - 1)) / totalSeries;
  ...
  private renderGroupedBars(
    ...
    bars.forEach((bar, barIndex) => {
      ...
      data.forEach((d, i) => {
        ...
        if (labelCfg) {
          const configs = Array.isArray(labelCfg)
            ? labelCfg
            : typeof labelCfg === 'object'
              ? [labelCfg]
              : [{}];
          configs.forEach((c) =>
            this.renderBarLabel(bar, c, x, y, width, height, value, i, isHorizontal, d, cellFill),
          );
        }
      });
    });
  ...
  private renderBarRect(
    ...
    if (hasVariableRadius && hasAnyRadius) {
      this.renderBarPath(group, bar, x, y, width, height, index, [topLeft, topRight, bottomRight, bottomLeft], fillOverride);
    } else {
  ...
  onChartMouseMove(event: MouseEvent): void {
    ...
    const { dataIndex, dataPoint } = result;
    ...
    const position = isHorizontal
      ? { x: mouseX, y: bandPos + bandwidth / 2 }
      : { x: bandPos + bandwidth / 2, y: mouseY };

    // Build tooltip payload from all visible bars
    const payload = this.bars()
      .filter((bar) => !bar.hide())
      .map((bar) => ({
  ...
  private hasHoverEffectsConfigured(): boolean {
    return anyElementHasHoverEffects(
      this.bars(),
      (bar) => !bar.hide()
    );
  }
