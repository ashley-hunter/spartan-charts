...
export class SpnLineChart<T = unknown> implements AfterContentInit {
  ...
  private renderAxes(): void {
    ...
    this.xAxes().forEach((xAxis) => {
      ...
      renderXAxis(
        ...
        scale,
        innerHeight,
        this.chartContext.margin().right,
      );
    });
    ...
    this.yAxes().forEach((yAxis) => {
      ...
      const domain = this.calculateYAxisDomainFromLines(
        data,
        yAxis.axisId(),
        yAxis.domain(),
      );
  ...
  private calculateYAxisDomainFromLines(
    ...
    return calculateSeriesYDomain(
      data,
      toSeriesConfig(this.lines()),
      axisId,
      domainInput,
      { supportStacking: false, includeZero: false },
    );
  }
  ...
  private renderDots(): void {
    ...
    this.lines().forEach((line) => {
      ...
      data.forEach((d, i) => {
        ...
        const yPos = (yScale as d3.ScaleLinear<number, number>)(
          yValue as number,
        );
  ...
  private updateHoverStyles(): void {
    ...
    const allLinePaths = d3
      .select(this.linesLayer)
      .selectAll<SVGPathElement, unknown>('.line-path');
