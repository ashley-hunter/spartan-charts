...
export class SpnTooltip {
  ...
  // Cursor line position and height (from chart context, scaled for responsive mode)
  readonly cursorTop = computed(() => {
    return this.chartContext.margin().top * this.chartContext.scaleY();
  });

  readonly cursorHeight = computed(() => {
    return this.chartContext.innerHeight() * this.chartContext.scaleY();
  });

  // Whether to show cursor - respects both the input and chart type (e.g., disabled for pie charts)
  readonly showCursor = computed(() => {
    return this.cursor() && this.chartContext.showCursorLine();
  });
