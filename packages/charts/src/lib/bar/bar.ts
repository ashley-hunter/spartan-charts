...
import { BarLabelConfig, BarRadius, DataKey } from '../types';
...
export class SpnBar implements OnDestroy {
  ...
  readonly radius = input<BarRadius>(0);
  ...
  readonly barSize = input<number>(); // Override container barSize
  readonly maxBarSize = input<number>();
  readonly minPointSize = input(0, { transform: numberAttribute });
  ...
  readonly backgroundFill = input<string>('#f5f5f5');
