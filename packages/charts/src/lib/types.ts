...
export function isTimeScale(
  scale: Scale,
): scale is d3.ScaleTime<number, number> {
...
export function getUniqueValues<T>(
  ...
  return Array.from(
    new Set(data.map((item) => getValueByDataKey(item, dataKey))),
  );
}
...
export function calculateDomain<T>(
  ...
  const values = data
    .map((item) => getValueByDataKey(item, dataKey))
    .filter((v) => v != null);
  ...
  return [
    d3.min(values as number[]) as number,
    d3.max(values as number[]) as number,
  ];
}
