import { ChartData, DataKey, Domain, getValueByDataKey } from '../types';
...
export function calculateSeriesYDomain<T>(
  ...
  const seriesForAxis = series.filter(
    (s) => !s.hide && s.yAxisId === axisId
  );
 ...
 */
export function toSeriesConfig<T, C extends {
  dataKey: () => DataKey<T>;
  hide: () => boolean;
  yAxisId: () => string;
