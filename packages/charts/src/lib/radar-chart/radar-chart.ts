...
export class SpnRadarChart<T = unknown> implements AfterContentInit {
  ...
  readonly radiusAxisStroke = input<string>('hsl(var(--foreground))');
  ...
  readonly axisLabel = input<
    ((datum: T, index: number) => RadarAxisLabelLine[]) | undefined
  >(undefined);
  ...
  readonly centerX = computed(() => {
    const m = this.margin();
    return m.left + (this.width() - m.left - m.right) / 2;
  });
  readonly centerY = computed(() => {
    const m = this.margin();
    return m.top + (this.height() - m.top - m.bottom) / 2;
  });
  ...
  constructor() {
    ...
    effect(() => {
      ...
      this.chartContext.width.set(this.width());
      this.chartContext.height.set(this.height());
  ...
  private adaptiveStep(roughStep: number, correctionFactor: number): number {
    ...
    const amendStepRatio =
      (Math.ceil(stepRatio / stepRatioScale) + correctionFactor) * stepRatioScale;
  ...
  private renderGrid(outerR: number, n: number): void {
    ...
    data.forEach((d, k) => {
      ...
      if (custom) {
        ...
        lines.forEach((seg) => {
          ...
          if (seg.muted) t.attr('fill', 'hsl(var(--muted-foreground))');
      ...
      if (s.isAnimationActive()) {
        path
          ...
          .attrTween('d', () => (t: number) =>
            this.closedPath(finalPts.map(([x, y]) => [x * t, y * t] as [number, number])),
          );
      } else {
  ...
  onChartMouseMove(event: MouseEvent): void {
    ...
    const label = String(
      getValueByDataKey(this.data()[best], this.dataKey()),
    );
    updateTooltip(
