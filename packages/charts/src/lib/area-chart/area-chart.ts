...
export class SpnAreaChart<T = unknown> implements AfterContentInit {
  ...
  private readonly revealRect =
    viewChild<ElementRef<SVGRectElement>>('revealRect');
  ...
  private renderAxes(): void {
    ...
    this.xAxes().forEach((xAxis) => {
      ...
      const domain = calculateAxisDomain(
        data,
        xAxis.dataKey(),
        xAxis.type(),
        xAxis.domain(),
    ...
    this.yAxes().forEach((yAxis) => {
      ...
      let domain = this.calculateYAxisDomainFromAreas(
        data,
        yAxis.axisId(),
        yAxis.domain(),
      );
  ...
  private ensureDefaultScales(): void {
    ...
    const hasDefaultX = this.xAxes().some(
      (a) => a.axisId() === 'default' && !a.hide(),
    );
    if (!hasDefaultX) {
    ...
    const hasDefaultY = this.yAxes().some(
      (a) => a.axisId() === 'default' && !a.hide(),
    );
    if (!hasDefaultY) {
      ...
      if (this.stackOffset() === 'expand') {
        ...
        const domain = this.calculateYAxisDomainFromAreas(
          data,
          'default',
          ['auto', 'auto'],
        );
  ...
  private calculateYAxisDomainFromAreas(
    ...
    return calculateSeriesYDomain(
      data,
      toSeriesConfig(this.areas()),
      axisId,
      domainInput,
      { supportStacking: true, includeZero: true }
    );
  }
  ...
  private revealOnce(): void {
    ...
    if (!rect) return;
    const animated = this.areas().filter((a) => a.isAnimationActive());
  ...
  onChartMouseMove(event: MouseEvent): void {
    ...
    const { dataIndex, dataPoint } = findNearestDataPoint(
      mouseX,
      data,
      xScale,
      xAxisDataKey
    ...
    const label = xAxisDataKey
      ? String(getValueByDataKey(dataPoint, xAxisDataKey))
      : String(dataIndex);

    updateTooltip(
      this.chartContext as ChartContextService<any>,
      dataIndex,
      // x snaps to the data point (cursor line / active dot), y follows the cursor (recharts-style)
